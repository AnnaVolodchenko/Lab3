# lab 3
Володченко Анна ПМ-2401. Вариант: 5
...
3. **Класс BookList**
   ...
   3. Методы:
      - addBook(Book book) — добавляет новую книгу в коллекцию. Если массив переполняется, его размер увеличивается в два раза.
      - getBook(String title) — ищет и возвращает книгу по названию. Возвращает null, если книга не найдена.
...
public class Test {
    public static void main(String[] args) {
        ...
        String bookTime = "'В поисках утраченного времени'";
        Book book = library.getBook(bookTime);
        if (book != null) {
            double time = (double) book.getPages() / user.getReadingSpeed();
            System.out.printf("На прочтение книги"+ bookTime+ " потребуется %.2f часов%n",time);
        } else {
            System.out.println("Книга "+bookTime+" не найдена.");
        }
